...
private struct EditTrackedComponent: Identifiable, Hashable {
    let id: String
    let label: String
    let subtitle: String
...
private func editIconForKey(_ key: String) -> String {
    switch key {
...
private func editColorFromHex(_ hex: String, fallback: Color = .sgPrimary) -> Color {
...
private extension EditServiceViewModelComponentOption {
    func toLocal() -> EditTrackedComponent {
        EditTrackedComponent(
            id: id,
            label: label,
            subtitle: intervalLabel,
            iconUnicode: editIconForKey(iconKey),
            color: editColorFromHex(colorHex)
        )
    }
}
...
struct EditServiceView: View {
    ...
    private var selectedOption: VehicleOption? {
        ...
        return vehicleOptions.first { $0.id == id }
    }
    ...
    private var availableComponents: [EditTrackedComponent] {
        (model.state.availableComponents as [EditServiceViewModelComponentOption]).map { $0.toLocal() }
    }
        ...
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    EditFieldLabel(text: "Kendaraan")
                    if let option = selectedOption {
                        VehiclePickerRow(selected: option, locked: false) {
                            showVehiclePicker = true
                        }
                        .padding(.bottom, 18)
                    }

                    EditFieldLabel(text: "Jenis catatan")
                    EditModeSelector(
                    ...
                    case ServiceKind.komponen:
                        EditFieldLabel(text: "Komponen yang diservis · \(model.state.selectedComponentIds.count)")
                        if model.state.hasTrackedComponents {
                            EditComponentChipsRow(
...
private struct EditComponentChipsRow: View {
    let selectedIds: Set<String>
    let allItems: [EditTrackedComponent]
    let onRemove: (String) -> Void
    let onAdd: () -> Void

    private var pickedItems: [EditTrackedComponent] {
...
private struct EditComponentPickerSheet: View {
    let allItems: [EditTrackedComponent]
    ...
    private var filtered: [EditTrackedComponent] {
...
private struct EditFieldLabel: View {
    let text: String
    var body: some View {
        Text(text)
...
private struct EditModeSelector: View {
...
private struct EditEditableRowField: View {
    let label: String
...
private struct DateRowFieldEdit: View {
    let label: String
...
private struct EditAutoReminderInfoCard: View {
    var body: some View {
...
private struct EditServiceSaveBar: View {
    let onSave: () -> Void
    var enabled: Bool = true

    var body: some View {
        VStack {
            AppButton(title: "Simpan Perubahan", action: onSave, isEnabled: enabled)
        }
